...
def plot_count_to_infinity():
    ...
    # Pad shorter list so both have the same length for clear comparison
    max_len = max(len(no_poison_hops), len(poison_hops))
    while len(no_poison_hops) < max_len:
        no_poison_hops.append(no_poison_hops[-1])
    while len(poison_hops) < max_len:
        poison_hops.append(poison_hops[-1])
    ...
    ax.plot(rounds, no_poison_hops, marker="o", linewidth=2.5,
            color="#E84C4C", label="Without Poison Reverse — settles via alternate path (B→C→D→A)")
    ax.plot(rounds, poison_hops, marker="s", linewidth=2.5,
            color="#4CE87A", label="With Poison Reverse — route marked ∞ immediately")

    ax.axhline(y=INFINITY, color=TEXT, linestyle="--", linewidth=1, alpha=0.4)
    ax.text(0.02, INFINITY + 0.3,
    ...
    ax.set_title("Link Failure A–B: Without vs With Poison Reverse",
    ...
    # Annotate key events
    ax.annotate("A–B link fails\n(stale routes present)", xy=(0, no_poison_hops[0]),
                xytext=(0.3, no_poison_hops[0] + 2),
                color=TEXT, fontsize=9, alpha=0.8,
                arrowprops=dict(arrowstyle="->", color=TEXT, alpha=0.5))

    legend = ax.legend(facecolor="#161b22", edgecolor=GRID, labelcolor=TEXT, fontsize=10,
                       loc="center right")
    plt.tight_layout()
