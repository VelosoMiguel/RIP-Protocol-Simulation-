...
def bellman_ford_round(table, routers, links, poison_reverse=False, verbose=True):
    ...
    # Invalidate routes whose next-hop is no longer a neighbour (link failure)
    for router in routers:
        for dest in routers:
            cost, next_hop, src = new_table[router][dest]
            if next_hop != "-" and next_hop not in neighbours[router]:
                if cost < INFINITY:
                    new_table[router][dest] = (INFINITY, "-", "Unknown")
                    changes.append((router, dest, cost, INFINITY, "-"))
                    changed = True
    ...
    if verbose and changes:
        for router, dest, old_cost, new_cost, via in changes:
            old_str = "∞" if old_cost >= INFINITY else str(old_cost)
            new_str = "∞" if new_cost >= INFINITY else str(new_cost)
            print(f"  [UPDATE] Router {router} -> {dest}: {old_str} hops → {new_str} hops via {via}")
    elif verbose:
        print("  No changes.")
...
def simulate_failure(poison_reverse=False, verbose=True):
    ...
    First converges the full square topology, then removes the A-B link
    so routers start with stale routes — correctly demonstrating count-to-infinity.
    """
    routers = ["A", "B", "C", "D"]
    links_full = [("A", "B"), ("B", "C"), ("C", "D"), ("D", "A")]
    links_after_failure = [("B", "C"), ("C", "D"), ("D", "A")]

    label = "With Poison Reverse" if poison_reverse else "Without Poison Reverse"

    # --- Phase 1: converge on full topology (silent) ---
    table = make_table(routers)
    set_direct_links(table, links_full)
    for _ in range(10):
        table, changed = bellman_ford_round(table, routers, links_full,
                                            poison_reverse=False, verbose=False)
        if not changed:
            break

    # Remove A-B link: routers still have stale routes pointing through A-B
    # Clear only the direct entries for the failed link
    table["A"]["B"] = (INFINITY, "-", "Unknown")
    table["B"]["A"] = (INFINITY, "-", "Unknown")
    ...
    if verbose:
        ...
        print("Network was converged, then A-B link removed.")
        print("=" * 60)
        print_table(table, "Round 0 (post-failure, stale routes present)")
